  ...
  return (
    <div className="flex">
      ...
      <main className="flex-1 px-8 py-10 max-w-2xl">
        <h1 className="font-display text-2xl font-semibold mb-1">Compose campaign</h1>
        <p className="text-muted text-sm mb-8">
          Use <code className="text-accent">{'{name}'}</code>, <code className="text-accent">{'{email}'}</code>, or any
          other column header from your sheet as a placeholder.
        </p>

        <form onSubmit={handleCreate} className="space-y-5">
          <div>
            <label className="block text-sm text-muted mb-1">Campaign name (internal)</label>
            <input required value={name} onChange={(e) => setName(e.target.value)} className="input" placeholder="August product update" />
          </div>

          <div>
            <label className="block text-sm text-muted mb-1">Subject</label>
            <input required value={subject} onChange={(e) => setSubject(e.target.value)} className="input" placeholder="Hi {name}, quick update" />
          </div>
          ...
          <div>
            <label className="block text-sm text-muted mb-1">
              Recipients — Excel file (<code>name</code>, <code>email</code> columns, plus any extras)
            </label>
            <input
              type="file"
              required
          ...
          <div>
            <label className="block text-sm text-muted mb-1">Attachments (optional — documents, images, video)</label>
            <input
              type="file"
              multiple
          ...
          <button type="submit" className="bg-accent hover:bg-accent/90 text-white font-medium text-sm rounded-md px-5 py-2.5 transition-colors">
            Create campaign
          </button>
          ...
          {importSummary && (
            <p className="text-sm text-sent">
              ...
              {importSummary.skipped > 0 && ` · skipped ${importSummary.skipped} invalid rows`}
            </p>
          )}
        </form>
      </main>
